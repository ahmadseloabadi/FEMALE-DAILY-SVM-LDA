...
dfsmote = pd.read_csv('data/smote/data_smote.csv')
...
if not st.sidebar.checkbox("Hide", True, key='hide_checkbox_dashboard'):
    ...
    if report_option == 'Confusion Matrix':
        ...
        confusion_option=st.selectbox('dataset option',['sesudah_smote','sebelum_smote','sesudah_smote_topik','sebelum_smote_topik'],key='confusion')
        
        if confusion_option == 'sesudah_smote':
           rt.plot_confusion(confusion_option,123,15,9,113)
        if confusion_option == 'sebelum_smote':
            rt.plot_confusion(confusion_option,124,10,26,40)
        if confusion_option == 'sesudah_smote_topik':
           rt.plot_confusion_topik(confusion_option,123,15,9,113)
        if confusion_option == 'sebelum_smote_topik':
            rt.plot_confusion_topik(confusion_option,124,10,26,40)

    if report_option == 'K-fold':
        st.subheader(f'Tampilan plot {report_option}')
        ######################### Cross Validation #########################
        kfold_option=st.selectbox('dataset option',['sesudah_smote','sebelum_smote','sebelum_smote_topik','sesudah_smote_topik'],key='kfold')
        if kfold_option == 'sesudah_smote':
            rt.plot_kfold(kfold_option)
        if kfold_option == 'sebelum_smote':
            rt.plot_kfold(kfold_option)
        if kfold_option == 'sesudah_smote_topik':
            rt.plot_kfold_topik(kfold_option)
        if kfold_option == 'sebelum_smote_topik':
            rt.plot_kfold_topik(kfold_option)
######################### Dashboard Report #########################
